import{j as t}from"./jsx-runtime-u17CrQMm.js";import{r as c}from"./index-Bpooh2ix.js";import"./_commonjsHelpers-CqkleIqs.js";function te(s){const r=[],n=[];for(let e=s.length-1;e>=0;e--){const o=s[e];o&&n.push([o,0])}for(;n.length>0;){const e=n.pop();if(!e)break;const[o,a]=e;if(r.push({node:o,depth:a}),o.isOpen&&o.children.length>0)for(let i=o.children.length-1;i>=0;i--){const l=o.children[i];l&&n.push([l,a+1])}}return r}function P(s,r){const n=[...s];for(;n.length>0;){const e=n.pop();if(e){if(e.id===r)return e;for(let o=e.children.length-1;o>=0;o--){const a=e.children[o];a&&n.push(a)}}}return null}function S(s,r,n=!1){return{id:s,label:r,children:[],isOpen:!1,isSelected:!1,isIndeterminate:!1,isLoading:!1,hasChildren:n}}function re(s,r){const n=[],e=r.toLowerCase().trim();if(e==="")return n;function o(a,i){for(const l of a)l.label.toLowerCase().includes(e)&&n.push({node:l,depth:0,breadcrumb:i.length>0?i.join(" › "):void 0}),l.children.length>0&&o(l.children,[...i,l.label])}return o(s,[]),n}const ne=500;function oe(s,r){const n=4+Math.floor(Math.random()*5),e=[];for(let o=0;o<n;o++){const a=`${s}-${o}`,i=`${r} › Item ${o+1}`,l=Math.random()>.5;e.push(S(a,i,l))}return e}function se(s,r){return new Promise(n=>{setTimeout(()=>{const e=oe(s,r);n(e)},ne)})}function ae(){return["Engineering","Design","Product","Marketing","Sales","Operations","Finance","Human Resources"].map((r,n)=>S(`root-${n}`,r,!0))}function ie(s){const r=s?.fetchChildrenFn??se,[n,e]=c.useState(()=>s?.initialRootNodes??ae()),[o,a]=c.useState(null),i=c.useMemo(()=>te(n),[n]),l=c.useCallback((g,y)=>{e(b=>{function h(v){return v.map(w=>{const m={...w,children:h(w.children)};return m.id===g?y(m):m})}return h(b)})},[]),d=c.useCallback(g=>{const y=P(n,g);if(y){if(y.children.length>0){l(g,b=>({...b,isOpen:!b.isOpen}));return}y.hasChildren&&!y.isLoading&&(a(null),l(g,b=>({...b,isLoading:!0})),r(g,y.label).then(b=>{l(g,h=>({...h,children:b,isLoading:!1,isOpen:!0}))},b=>{l(g,v=>({...v,isLoading:!1}));const h=b instanceof Error?b.message:"Failed to load children";a(h)}))}},[n,l,r]),x=c.useCallback(g=>{e(y=>{const b=U(y),h=P(b,g);if(!h)return y;const v=!h.isSelected;return W(h,v),G(b),b})},[]),k=c.useMemo(()=>{const g=[];function y(b){for(const h of b)h.isSelected&&g.push({id:h.id,label:h.label}),!h.isSelected&&h.children.length>0&&y(h.children)}return y(n),g},[n]),N=c.useCallback(g=>{x(g)},[x]),C=c.useCallback(()=>{a(null)},[]);return{flatNodes:i,rootNodes:n,selectedNodes:k,error:o,toggleExpand:d,toggleSelect:x,deselectNode:N,clearError:C}}function U(s){return s.map(r=>({...r,children:U(r.children)}))}function W(s,r){s.isSelected=r,s.isIndeterminate=!1;for(const n of s.children)W(n,r)}function G(s){let r=0,n=0;for(const e of s){if(e.children.length>0){const[o,a]=G(e.children),i=o===a&&a>0,l=o===0,d=e.children.some(x=>x.isIndeterminate);i?(e.isSelected=!0,e.isIndeterminate=!1):l&&!d?(e.isSelected=!1,e.isIndeterminate=!1):(e.isSelected=!1,e.isIndeterminate=!0)}n++,e.isSelected&&r++}return[r,n]}function le({containerRef:s,itemCount:r,itemHeight:n,overscan:e=5}){const[o,a]=c.useState(0),[i,l]=c.useState(0),d=c.useRef(0),x=c.useRef(null),k=c.useCallback(()=>{if(!s.current)return;const m=s.current.scrollTop;d.current=m,x.current===null&&(x.current=requestAnimationFrame(()=>{a(m),x.current=null}))},[s]);c.useLayoutEffect(()=>{const m=s.current;if(!m)return;m.scrollTop!==d.current&&(a(m.scrollTop),d.current=m.scrollTop),l(m.clientHeight),m.addEventListener("scroll",k,{passive:!0});const j=new ResizeObserver(M=>{for(const T of M){const O=T.contentRect.height;l(_=>Math.abs(O-_)>.5?O:_)}});return j.observe(m),()=>{m.removeEventListener("scroll",k),j.disconnect(),x.current!==null&&cancelAnimationFrame(x.current)}},[s,k,r]);const N=r*n,C=n,g=i||320,y=Math.floor(o/C),b=Math.ceil(g/C),h=Math.max(0,y-e),v=Math.min(r-1,y+b+e),w=[];for(let m=h;m<=v;m++)w.push({index:m,offsetTop:m*C});return{virtualItems:w,totalHeight:N}}const de=20;function B({flatNode:s,offsetTop:r,rowHeight:n,isHighlighted:e,optionId:o,onToggleExpand:a,onToggleSelect:i,searchQuery:l}){const{node:d,depth:x}=s,k=l!==void 0,N=c.useCallback(h=>{h.stopPropagation(),h.preventDefault(),a(d.id)},[d.id,a]),C=c.useCallback(h=>{h.stopPropagation()},[]),g=c.useCallback(()=>{i(d.id)},[d.id,i]),y=c.useCallback(()=>{i(d.id)},[d.id,i]),b=k?12:12+x*de;return t.jsxs("div",{id:o,role:"option","aria-selected":d.isSelected,"aria-expanded":!k&&d.hasChildren?d.isOpen:void 0,onClick:y,className:["absolute left-0 right-0 flex cursor-pointer items-center text-sm","transition-colors duration-75",e?"bg-blue-50":"hover:bg-gray-50"].join(" "),style:{height:`${n}px`,top:`${r}px`,paddingLeft:`${b}px`,paddingRight:"12px"},children:[!k&&(d.hasChildren?t.jsx("button",{type:"button",tabIndex:-1,onClick:N,onMouseDown:h=>h.preventDefault(),className:["mr-1 flex h-5 w-5 flex-shrink-0 items-center justify-center","rounded text-gray-500 hover:bg-gray-100 hover:text-gray-700","transition-colors duration-100"].join(" "),"aria-label":d.isOpen?`Collapse ${d.label}`:`Expand ${d.label}`,"aria-hidden":"true",children:d.isLoading?t.jsx("span",{className:"block h-3 w-3 animate-pulse rounded-sm bg-gray-300"}):t.jsx("svg",{className:["h-3.5 w-3.5 transition-transform duration-150",d.isOpen?"rotate-90":""].join(" "),fill:"none",viewBox:"0 0 24 24",stroke:"currentColor",strokeWidth:2.5,"aria-hidden":"true",children:t.jsx("path",{strokeLinecap:"round",strokeLinejoin:"round",d:"M9 5l7 7-7 7"})})}):t.jsx("span",{className:"mr-1 inline-block h-5 w-5 flex-shrink-0","aria-hidden":"true"})),t.jsx("input",{type:"checkbox",tabIndex:-1,checked:d.isSelected,ref:h=>{h&&(h.indeterminate=d.isIndeterminate)},onClick:C,onChange:g,className:["mr-2 h-4 w-4 flex-shrink-0 cursor-pointer rounded","border-gray-300 text-blue-600","focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"].join(" "),"aria-label":`Select ${d.label}`}),t.jsxs("div",{className:"flex min-w-0 flex-1 flex-col justify-center overflow-hidden",children:[t.jsx("span",{className:["truncate",d.isSelected?"font-medium text-gray-900":"text-gray-700"].join(" "),children:l?t.jsx(ce,{text:d.label,query:l}):d.label}),s.breadcrumb&&t.jsx("span",{className:"truncate text-xs leading-tight text-gray-400",children:s.breadcrumb})]})]})}function ce({text:s,query:r}){const n=s.toLowerCase(),e=r.toLowerCase().trim(),o=n.indexOf(e);if(o===-1||e==="")return t.jsx(t.Fragment,{children:s});const a=s.slice(0,o),i=s.slice(o,o+e.length),l=s.slice(o+e.length);return t.jsxs(t.Fragment,{children:[a,t.jsx("mark",{className:"rounded-sm bg-yellow-100 text-gray-900",children:i}),l]})}B.__docgenInfo={description:`TreeRow — a single absolutely-positioned row inside the virtualized list.\r
\r
...
can interleave tags with the search input on the same line.`,methods:[],displayName:"SelectedTags",props:{selectedNodes:{required:!0,tsType:{name:"Array",elements:[{name:"SelectedNode"}],raw:"SelectedNode[]"},description:""},onRemove:{required:!0,tsType:{name:"signature",type:"function",raw:"(nodeId: string) => void",signature:{arguments:[{type:{name:"string"},name:"nodeId"}],return:{name:"void"}}},description:""}}};const ue=32,pe=44,z="hcb-option-";function K(s={}){const[r,n]=c.useState(!1),[e,o]=c.useState(-1),[a,i]=c.useState(""),l={};s.initialRootNodes&&(l.initialRootNodes=s.initialRootNodes),s.fetchChildrenFn&&(l.fetchChildrenFn=s.fetchChildrenFn);const{flatNodes:d,rootNodes:x,selectedNodes:k,error:N,toggleExpand:C,toggleSelect:g,deselectNode:y,clearError:b}=ie(l),h=c.useMemo(()=>a.trim().length>0?re(x,a):[],[x,a]),v=a.trim().length>0,w=c.useMemo(()=>(v?h:d).filter(f=>f&&f.node&&f.node.id),[v,h,d]),m=v?pe:ue,j=c.useRef(null),M=c.useRef(null),T=c.useRef(null),{virtualItems:O,totalHeight:_}=le({containerRef:j,itemCount:w.length,itemHeight:m,overscan:10}),$="hcb-listbox",Q=e>=0&&e<w.length?`${z}${w[e]?.node.id??""}`:void 0;c.useEffect(()=>{function u(f){const p=M.current;p&&(p.contains(f.target)||(n(!1),i(""),o(-1)))}return document.addEventListener("mousedown",u),()=>{document.removeEventListener("mousedown",u)}},[]),c.useEffect(()=>{r&&j.current&&requestAnimationFrame(()=>{j.current&&(j.current.scrollTop=0)})},[r]),c.useEffect(()=>{if(e<0||!j.current||w.length===0)return;const u=j.current,f=e*m,p=f+m,F=u.scrollTop,ee=F+u.clientHeight;f<F?requestAnimationFrame(()=>{u.scrollTop=f}):p>ee&&requestAnimationFrame(()=>{u.scrollTop=p-u.clientHeight})},[e,m,w.length]),c.useEffect(()=>{j.current&&(j.current.scrollTop=0)},[v]);const Y=c.useCallback(u=>{if(u.target===T.current){r||n(!0);return}T.current?.focus(),n(f=>{const p=!f;return p||(i(""),o(-1)),p})},[r]),V=c.useCallback(u=>{u.stopPropagation(),T.current?.focus(),n(f=>{const p=!f;return p||(i(""),o(-1)),p})},[]),J=c.useCallback(u=>{const f=u.target.value;i(f),o(-1),r||n(!0)},[r]),Z=c.useCallback(u=>{const f=w.length;switch(u.key){case"ArrowDown":{u.preventDefault(),r?o(p=>Math.min(p+1,f-1)):(n(!0),o(0));break}case"ArrowUp":{u.preventDefault(),r&&o(p=>Math.max(p-1,0));break}case"ArrowRight":{if(!v&&r&&e>=0){u.preventDefault();const p=w[e];p&&p.node.hasChildren&&!p.node.isOpen&&C(p.node.id)}break}case"ArrowLeft":{if(!v&&r&&e>=0){u.preventDefault();const p=w[e];p&&p.node.isOpen&&C(p.node.id)}break}case"Enter":{if(u.preventDefault(),!r)n(!0),o(0);else if(e>=0){const p=w[e];p&&g(p.node.id)}break}case"Home":{u.preventDefault(),r&&f>0&&o(0);break}case"End":{u.preventDefault(),r&&f>0&&o(f-1);break}case"Escape":{u.preventDefault(),v?(i(""),o(-1)):(n(!1),o(-1));break}}},[r,v,e,w,C,g]);return t.jsxs("div",{ref:M,className:"relative w-full max-w-lg",children:[t.jsxs("div",{onClick:Y,className:["flex min-h-[2.5rem] w-full cursor-text flex-wrap items-center gap-1","rounded-md border bg-white px-3 py-1.5","transition-all duration-150",r?"border-blue-500 shadow-sm":"border-gray-300 hover:border-gray-400"].join(" "),children:[k.length>0&&t.jsx(X,{selectedNodes:k,onRemove:y}),t.jsx("input",{ref:T,type:"text",role:"combobox","aria-expanded":r,"aria-haspopup":"listbox","aria-controls":r?$:void 0,"aria-activedescendant":r?Q:void 0,"aria-label":"Select items from hierarchical list",value:a,onChange:J,onKeyDown:Z,placeholder:k.length>0?"":"Type to search or click to browse…",autoComplete:"off",className:["min-w-[120px] flex-1 border-0 bg-transparent py-1 text-sm","text-gray-900 outline-none placeholder:text-gray-400"].join(" ")}),t.jsx("button",{type:"button",tabIndex:-1,onClick:V,className:"ml-auto flex-shrink-0 rounded p-1 text-gray-400 hover:text-gray-600 transition-colors","aria-label":r?"Close dropdown":"Open dropdown",children:t.jsx("svg",{className:["h-4 w-4 transition-transform duration-200",r?"rotate-180":""].join(" "),fill:"none",viewBox:"0 0 24 24",stroke:"currentColor",strokeWidth:2,"aria-hidden":"true",children:t.jsx("path",{strokeLinecap:"round",strokeLinejoin:"round",d:"M19 9l-7 7-7-7"})})})]}),r&&t.jsxs("div",{className:"absolute left-0 top-full z-50 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg",children:[v&&w.length===0&&t.jsxs("div",{className:"px-4 py-3 text-center text-sm text-gray-500",role:"status",children:["No results for “",a,"”"]}),t.jsx("div",{ref:j,id:$,role:"listbox","aria-label":"Hierarchical options","aria-multiselectable":"true",className:"relative overflow-y-auto overflow-x-hidden",style:{maxHeight:"320px",minHeight:w.length>0?"50px":"0px",willChange:"scrollTop"},children:w.length>0&&t.jsx("div",{className:"relative w-full",style:{height:`${_}px`},children:O.map(u=>{const f=w[u.index];return f?t.jsx(B,{flatNode:f,offsetTop:u.offsetTop,rowHeight:m,isHighlighted:u.index===e,optionId:`${z}${f.node.id}`,onToggleExpand:C,onToggleSelect:g,searchQuery:v?a:void 0},f.node.id):null})})}),!v&&d.length===0&&!N&&t.jsx("div",{className:"px-4 py-3 text-center text-sm text-gray-400",role:"status",children:"No items available"}),N&&t.jsxs("div",{role:"alert",className:"flex items-center justify-between border-t border-red-500/20 bg-red-50 px-3 py-2 text-sm text-red-600",children:[t.jsx("span",{children:N}),t.jsx("button",{type:"button",tabIndex:-1,onClick:b,className:"ml-2 text-xs font-medium underline hover:text-red-500","aria-label":"Dismiss error",children:"Dismiss"})]})]})]})}K.__docgenInfo={description:`HierarchicalCombobox — the root component.\r
\r
...
───────────────────\r
  ...
  Escape     → clears search query, then closes dropdown`,methods:[],displayName:"Combobox"};const ve={title:"Components/Combobox",component:K,parameters:{layout:"centered",docs:{description:{component:`Storybook stories for the HierarchicalCombobox.\r
...
instructions that guide the tester through the keyboard flow.`,...A.parameters?.docs?.description}}};H.parameters={...H.parameters,docs:{...H.parameters?.docs,source:{originalSource:`{
  args: {
    initialRootNodes: generateSearchableTree()
  }
}`,...H.parameters?.docs?.source},description:{story:`A pre-loaded 3-level tree for testing search with ancestry context.\r
Type "Frontend" to see it matched under every department with breadcrumbs.\r
Type "Member" to see leaf nodes with full ancestry paths.`,...H.parameters?.docs?.description}}};const ke=["Default","LargeDataset","AsyncLoading","ErrorState","HighContrast","KeyboardOnly","SearchFiltering"];export{R as AsyncLoading,E as Default,L as ErrorState,D as HighContrast,A as KeyboardOnly,I as LargeDataset,H as SearchFiltering,ke as __namedExportsOrder,ve as default};
